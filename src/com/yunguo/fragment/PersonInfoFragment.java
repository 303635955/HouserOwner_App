...
//				.findViewById(R.id.expandable_list_view);
	...
	public void initListView(List<Map<String, String>> houseinfoList2) {
		...
		map.put("use", "住宅");
		map.put("owner", "张三");
		map.put("call", "1222222");
		map.put("adress", "天禧路192号");
	...
	public void addItemByValue(Map<String, String> map) {
		...
		item.add(Floor +map.get("floor"));
		item.add(Number +map.get("number"));
		item.add(Use +map.get("use"));
		item.add(Owner +map.get("owner"));
		item.add(PHONE +map.get("call"));
		item.add(Adress +map.get("adress"));
		child.add(item);
	}
	...
	public void getHouseMessage(final String pramr, final Handler handler) {
		...
		new Thread(new Runnable() {
			...
			public void run() {
				...
				String url = "http://192.168.1.144:8118/HouseMobileApp/TenantInfo";

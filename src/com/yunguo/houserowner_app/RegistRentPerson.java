...
public class RegistRentPerson extends Activity {
	...
	public void setclick() {
		...
		btn.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				InputCheckout();
				if(form.validate() &&  FromUtil.date(carddate.getText().toString() , date.getText().toString())){
					...
					PitureBean.savafile().setIdCardNo(person_cardid.getText().toString());
					setsex(Sextext);
					setnation(nation);
	...
	public void setText() {
		if (s != null) {
			...
			if (s.get("Sex") != null && !s.get("Sex").equals("") && !(s.get("Sex").isEmpty())) {
				if (getsexselect(s.get("Sex"), mItems) != 200) {
					Sex.setSelection(getsexselect(s.get("Sex"), mItems), true);
				} else {
	...
	public int getsexselect(String str, String[] data) {
		int i;
		for (i = 0; i < data.length; i++) {
			if (str.equals(data[i])) {
				break;

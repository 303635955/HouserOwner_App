...
public class IDCardCode extends Activity implements OnClickListener{
	...
	private static int MAIN = 101;

	public static final int MAX_IDCARD_SIZE = 1920 * 1080;
	private Button btn_photo, btn_camrea;
	...
	public void setclick(){
		...
		Front_OriPath = DirPath + File.separator + "oriphoto.jpg";
		Back_OriPath = DirPath + File.separator + "oriphoto.jpg";
	...
	private void showFrontResult(Bundle bundle){
		...
		list.put("birth", bundle.getString("birth"));
		...
		switch(v.getId()){
			/*case R.id.btnPhoto:
				if (Environment.getExternalStorageState().equals(
					...
					Intent localIntent2 = Intent.createChooser(localIntent,
							"选择图片");
					startActivityForResult(localIntent2, REQUEST_CODE_GALLERY);
				} else {
			...
			case R.id.btn_recog_back:
				...
				if(list.size()>1 && list.size()<9){
					...
					Toast.makeText(getApplication(), "请先识别正面",
							Toast.LENGTH_SHORT).show();
				}
				break;
			case R.id.btn_jump:
